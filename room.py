...
# to close the server and deregister from discover server

def close():
    print("DeRegistering from Discovery")
    message = f"DEREGISTER {name}"
    room_socket.sendto(message.encode(), disco_server)
    print("closing room")
    sys.exit(0)
...
def main():
    ...
    parser.add_argument("-n", help="connecting room to the north", metavar="name of the room", default="")
    parser.add_argument("-s", help="connecting room to the south", metavar="name of the room", default="")
    parser.add_argument("-e", help="connecting room to the east", metavar="name of the room", default="")
    parser.add_argument("-w", help="connecting room to the west", metavar="name of the room", default="")
    parser.add_argument("-u", help="connecting room upwards", metavar="name of the room", default="")
    parser.add_argument("-d", help="connecting room downwards", metavar="name of the room", default="")
    ...
    room_server = ('localhost', room_socket.getsockname()[1])



    joinMessage = "join {}".format(room_server)
    message = "REGISTER room://localhost:{} {}".format(room_socket.getsockname()[1], name)

    room_socket.sendto(joinMessage.encode(), disco_server)
    joinMessageConfirmation, addr = room_socket.recvfrom(1024)

    if joinMessageConfirmation.decode() == "NOTOK":
        print("something went wrong when trying to join discovery")
        close()

    room_socket.sendto(message.encode(), disco_server)
    requestMessageConfirmation, addr = room_socket.recvfrom(1024)

    if requestMessageConfirmation.decode() == "NOTOK":
        print("something went wrong, closing room")
        close()
